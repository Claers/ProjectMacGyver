...
class App:
    ...
    def on_init(self):  # Init Window
        ...
        self._window = pygame.display.set_mode(self.size, HWSURFACE|DOUBLEBUF)
    ...
    def on_event(self, event):  # Each time that a event is caught in on_execute it check here for actions
        ...
        if event.type == VIDEORESIZE:  # Refresh window size when resize event is caught
            self._window = pygame.display.set_mode(event.dict['size'], HWSURFACE|DOUBLEBUF)
    ...
    def on_gui(self):
        self.label = self.textfont.render("Loot Collected : " + str(self.playerobj.lootcollected) + "/" + str(self.objnumber), 1, (255,255,255))
        if self.win:
            self.wintext = self.textfont.render("YOU WIN", 1, (255,255,255))
        if self.loose:
            self.loosetext = self.textfont.render("YOU LOOSE", 1, (255,255,255))


    def on_loop(self):
        if(len(self.loots) != 0):  # Check if loot is all collected or not
            if(self.oldposx != self.playerobj.posx or self.oldposy != self.playerobj.posy):  # Making a simple check with olppos vars to avoid collision check every frame
                ObjCollision = TestObjectCollision(self.playerobj, self.loots)  # Use The Function to test collision between player and objects
                if(ObjCollision != None):  # If there's a collision
                    if (ObjCollision[0] == "True"):
                        self.playerobj.lootcollected += 1
                        self.loots.remove(ObjCollision[1])  # Remove the loot from the list
                        self.obj_group.remove(ObjCollision[1])  # Remove the loot from the render
                self.oldposx = self.playerobj.posx  # Update position when it have changed
                self.oldposy = self.playerobj.posy

        if(self.playerobj.posx == self.guard.posx-30 and self.playerobj.posy == self.guard.posy): # If player is front of the guard / HARDCODED : The test check only left of the guard
            if(len(self.loots) == 0):
                self.win = True
    ...
    def on_render(self):  # The render of the game each frames
        ...
        if self.win:
            self._window.blit(self.wintext,(self.size[1]/2,self.size[0]/2))
        if self.loose:
            self._window.blit(self.loosetext, (self.size[1]/2,self.size[0]/2))
        pygame.display.flip()
        self.update = False
            ...
            for lenght in content:
                ...
                for casetype in lenght:
                    ...
                    if(casetype == 0):  # type 0 is wall
                        wall = Wall(x*30,y*30)
                        self.font_group.add(wall)  # add wall to background sprite group
                        self.COLLISION.update({(wall.posx,wall.posy) : 0})
                    elif(casetype == 1):  # type 1 is path
                        path = Path(x*30,y*30)
                        self.font_group.add(path)  # add path to background sprite group
